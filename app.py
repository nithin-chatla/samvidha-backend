...
def scrape_course_content(session):
    try:
        ...
        for tr in table.find_all("tr"):
            ...
            if s3_match: pdf_link = s3_match.group(1).replace('\\/', '/')
            
            yt_match = re.search(r'(https?://(?:www\.)?(?:youtube\.com/watch\?v=|youtu\.be/)[^\s"\'<>]+)', row_html, re.IGNORECASE)
            if yt_match: youtube_link = yt_match.group(1).replace('\\/', '/')
            ...
            for link in tr.find_all("a", href=True):
                ...
                if href and not href.startswith('#') and 'javascript' not in href.lower():
                    ...
                    low = full_href.lower()
                    if ".pdf" in low and not pdf_link: pdf_link = full_href
                    elif ("youtu.be" in low or "youtube.com" in low) and not youtube_link: youtube_link = full_href
                    else: extra_links.append({"text": link.get_text(strip=True), "href": full_href})

            if len(cols) > 6 and not pdf_link:
                ppt_text = cols[6].get_text(strip=True)
                if ppt_text.lower() not in ["nil", "-", ""]: powerpoint_text = ppt_text
    ...
    except Exception as e: return {"records": []}
