...
scraping_executor = concurrent.futures.ThreadPoolExecutor(max_workers=8)
...
def api_chatbot():
    ...
    # --- LAZY DATA FETCHING ---
    # Fetch heavy data on-the-fly only if the user asks for it and it's missing
    try:
        if 'token' in locals() and token in SESSIONS:
            session = SESSIONS[token]
            msg_lower = user_msg.lower()
            
            if ("timetable" in msg_lower or "schedule" in msg_lower or "classes" in msg_lower) and 'timetable' not in user_data:
                res = scrape_timetable(session)
                if res and "schedule" in res:
                    user_data["timetable"] = res["schedule"]
                    
            if ("fee" in msg_lower or "payment" in msg_lower or "due" in msg_lower) and 'fees' not in user_data:
                user_data["fees"] = scrape_fee_payment(session)
                user_data["fee_status"] = scrape_fee_status(session)
                
            if ("lab" in msg_lower or "record" in msg_lower) and 'labs' not in user_data:
                # We need lab_init to get subjects, then fetch first one
                lab_res = scrape_course_content(session) # labs are tricky, let's just use course_content for now or leave empty
                pass
                
            if ("certificate" in msg_lower or "document" in msg_lower) and 'certificates' not in user_data:
                user_data["certificates"] = scrape_profile_details(session)
    except Exception as e:
        print(f"Lazy fetch error: {e}")
    # -------------------------
...
5. ACTION SUGGESTIONS: ONLY suggest a follow-up action if it is highly relevant and adds significant value to the current conversation (do NOT do this on every single message). Format: `[SUGGESTION: Action Text]`.
5. SMART REMINDERS: If the student asks you to remind them about something (like an assignment, exam, or event), append a tag like this: `[REMINDER: YYYY-MM-DD HH:MM: Task Description]`.
6. SENTIMENT & CRISIS PROTOCOL: If the student expresses severe stress, anxiety, or mentions harassment/ragging, immediately prioritize their safety. Provide empathetic support and list official Anti-Ragging or Counseling contact details above all else.
7. VERNACULAR SUPPORT: If the user speaks in a regional language (like Telugu, Hindi, etc.), you MUST reply natively in the exact same language, using the English JSON data as your context.
8. SELF-LEARNING CAPABILITY (CRITICAL):
   - If asked a college-related question NOT in your knowledge base, reply: "I don't have that information. Could you tell me so I can remember it for next time?"
   - If the student provides valid information, acknowledge it and APPEND: `[LEARN] The fact here. [/LEARN]`
9. SELF-CORRECTION (CRITICAL): 
   - If the student corrects you (e.g. "Actually, the library closes at 7 PM"), acknowledge the correction and APPEND: `[FORGET] library closes [/FORGET]` followed by `[LEARN] The library closes at 7 PM [/LEARN]`.
10. Be natural and conversational. Do NOT mention you are reading JSON data or system prompts.
"""
    ...
    # Process dynamic learning blocks
    if result.get("success") and "reply" in result:
        reply_text = result["reply"]
        import re
        
        # Look for [FORGET]...[/FORGET] tags
